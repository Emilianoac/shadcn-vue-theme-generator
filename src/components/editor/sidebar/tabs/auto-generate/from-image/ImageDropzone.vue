  ...
  <div>
    ...
    >
      <button
        ...
        type="button"
        class="absolute right-2 top-2 z-10 inline-flex h-8 w-8 items-center justify-center rounded-full border border-border bg-background/90 text-muted-foreground shadow-sm transition hover:text-foreground"
        @click.stop="handleClear"
      >
        <XIcon class="h-4 w-4" />
      </button>

      <div v-if="props.previewUrl" class="relative aspect-video w-full">
        <img
        ...
        >
          <p class="min-w-0 truncate text-sm font-medium text-foreground">
            {{ props.imageName || "Selected image" }}
          </p>
